...
if __name__ == "__main__":
  ...
  # Διάβασμα των 2 αρχείων JSON, το ένα με το GDP και το άλλο με το POP και εμφάνιση χύμα στην οθόνη.
	dfFromTxt1=spark.read.text("./drive/MyDrive/US.NY.GDP.MKTP.CD.json")
	dfFromTxt2=spark.read.text("./drive/MyDrive/US.SP.POP.json")
	print("Το 1ο αρχείο προς εξέταση πριν από την επεξεργασία του, δηλαδή το US.NY.GDP.MKTP.CD.json")
	dfFromTxt1.show(truncate=False)
	print("Το 2ο αρχείο προς εξέταση πριν από την επεξεργασία του, δηλαδή το US.SP.POP.json")
  ...
  # Αφαιρείται η 1η γραμμή που αφορά την επικεφαλίδα του JSON επειδή δεν μας χρειάζεται και επίσης προκαλεί πρόβλημα στην ανάγνωση.
	...
	dfFromTxt1 = dfFromTxt1.withColumn('value', regexp_replace('value', '"value":"United States"','"countryValue":"United States"'))
  ...
  # που αντιπροσωπεύει μία νέα στήλη με μηδενικά όπου εκεί θα αποθηκευτεί το gdpPerCapita για αυτό τον λόγο θα μετονομαστεί έτσι.
	...
	dfFromTxt2 = dfFromTxt2.withColumn('value', regexp_replace('value', '"value":"United States"','"countryValue":"United States"'))
